...
class HBController1(Node):
    def __init__(self):
        ...
        self.vel_right_msg = self.create_publisher(
    ...
    def calculate_velocity_commands(self, x, y, th):
        ...
        self.cmd_vel_pub.publish(self.left_pub)
...
def main(args=None):
    ...
    while rclpy.ok():
        first = time.time()
        if len(hb_controller1.bot_1_x) != 0 and len(hb_controller1.bot_1_y) != 0 and len(hb_controller1.bot_1_theta) != 0 and (first - third) > 1:
