...
class HBController(Node):
    ...
    def inverse_kinematics(self, x, y, theta):
        ...
        matrix_3x3 = -np.array([[7.145919679862797, -12.376237623762377,-4.8615170766646765],[7.145919679862797, 12.376237623762377,-4.8615170766646765],[-14.291839359725595, 0.0, -4.861517076]])
...
def main(args=None):
    ...
    while rclpy.ok():
        while x_goal != 0 and y_goal != 0:
            ...
            # y_goal = 0
            # Apply appropriate force vectors
            # hb_controller.v_left, hb_controller.v_right, hb_controller.v_rear = hb_controller.inverse_kinematics(
            # hb_controller.vel_x, hb_controller.vel_y, hb_controller.vel_theta)
            result = hb_controller.inverse_kinematics(5,5,0)
            hb_controller.vel_left_msg.force.y = result[1]
            hb_controller.vel_right_msg.force.y = result[0]
            hb_controller.vel_rear_msg.force.y = result[2]
            ...
            hb_controller.get_logger().info("vel published")
            while hb_controller.distance(x_goal, y_goal) < 100:
